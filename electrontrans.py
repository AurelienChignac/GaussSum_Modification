...
########################################################################################################################
#                                                                                                                      #
#                         This program was modified by AURELIEN CHIGNAC on 27/06/2020                                  #
#        to generate a file 'ECDSpectrum.txt' containing ECD data (Energy (cm-1), Wavelength (nm) and ECD)             #
#         and a file Transitions.txt containing transitions data (Wavelength (nm), Energy (cm-1) and R)                #
#                                                                                                                      #
########################################################################################################################
...
#######################################################
        ...
        # Write "ECDSpectra.txt" containing info on the CD spectrum
        screen.write("Writing the ECD data to ECDSpectrum.txt\n")
        outputfile1=open(os.path.join(gaussdir,"ECDSpectrum.txt"),"w")
        outputfile1.write("Energy (cm-1)\tWavelength (nm)\tAbs\n")
        screen.write("Writing the Transitions to Transitions.txt\n")
        outputfile2=open(os.path.join(gaussdir,"Transitions.txt"),"w")
        outputfile2.write("        Wavelength (nm)\tEnergy (cm-1)\tR(length)")
        ...
        for x in range(numpts):
            ...
            realx = width * x / numpts + startwaveno
            outputfile1.write("%f\t%f\t%f" % (realx, 1.0e7/realx, t.spectrum[0,x]))
            if x < len(logfile.etenergies): # Write the R values out also
                outputfile2.write( "\t\n\t%f\t%f\t%f" % (convertor(logfile.etenergies[x], "cm-1", "nm"),
